...
1. **Plain SELECT only.** No `CREATE` / `INSERT` / `UPDATE` / `DELETE` / `MERGE` / `SET` inside
   ...
   The **one** documented exception is `storage_breakdown_analyze` (an `ANALYZE TABLE` template):
   it carries `runnable: false` so it is excluded from the read-only runner; sweeping it across
   tables is the job of the destructive, opt-in `tools/run_analyze.py`.
