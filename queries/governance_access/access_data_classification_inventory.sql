...
-- Scans run on serverless compute and complete asynchronously (new objects within ~24h), so a catalog with everything enabled but no serverless available, or only just enabled, can still show no rows yet.
