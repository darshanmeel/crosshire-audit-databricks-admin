...
| 07 | [`storage_breakdown_analyze`](https://learn.crosshire.ch/learn/tech/databricks/audit#q-storage_breakdown_analyze) | A single row of one Delta table's true on-disk footprint, split into active data, time-travel history, and reclaimable (vacuumable) bytes and file counts. **⚠️ The one non-`SELECT` query** — an `ANALYZE TABLE` template (`runnable: false`), excluded from `run_audit.py`. |
...
> ⚠️ **Caution — `storage_breakdown_analyze` runs `ANALYZE TABLE … COMPUTE STORAGE METRICS`, not a `SELECT`.**
> It's a copy-paste-only template (`runnable: false`) against a placeholder table, so the read-only
> `run_audit.py` skips it. Substitute your own `catalog.schema.table` and run it by hand, or sweep many
> tables with the **destructive, opt-in** [`tools/run_analyze.py`](../../tools/run_analyze.py) — dry-run by
> default; it executes only with `--run --yes`.
